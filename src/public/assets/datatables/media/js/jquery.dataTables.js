...
(function (factory) {
  ...
  var DataTable = function (selector, options) {
    ...
    /**
		 ...
		 *  @return {object} jQuery object, filtered by the given selector.
		 *  @dtopt API
    ...
    /**
		 ...
		 *    ("current") or not ("all"). If 'current' is given, then order is assumed to be
		 *    'current' and filter is 'applied', regardless of what they might be given as.
		 *  @return {array} Data for the matched elements. If any elements, as a result of the
    ...
    /**
		 ...
		 *  @return {array} An array of integers, representing the list of indexes in
    ...
    this.fnAddData = function (data, redraw) {
      ...
      const rows = Array.isArray(data) && (Array.isArray(data[0]) || $.isPlainObject(data[0]))
        ? api.rows.add(data)
        : api.row.add(data);
    ...
    /**
		 ...
		 *  @return {int} 0 on success, or 1 if failed (can't find the row)
		 *  @dtopt API
		 *  @deprecated Since v1.10
		 *
		 *  @example
    ...
    /**
		 ...
		 *  @param {bool} [redraw=true] Redraw the table or not
		 *  @return {array} The row that was deleted
		 *  @dtopt API
		 *  @deprecated Since v1.10
		 *
		 *  @example
    ...
    /**
		 ...
		 *  @return {array|object|string} If mRow is undefined, then the data for all rows is
    ...
    /**
		 ...
		 *  @return {array|node} If iRow is undefined, returns an array of all TR elements
    ...
    /**
		 ...
		 *  @return {int} If nNode is given as a TR, then a single index is returned, or
    ...
    /**
		 ...
		 *  @return {boolean} true if the row is currently open, false otherwise
		 *  @dtopt API
		 *  @deprecated Since v1.10
		 *
		 *  @example
    ...
    /**
		 ...
		 *  @return {node} The row opened. Note that if the table row passed in as the
    ...
    /**
		 ...
		 *  @return {object} DataTables settings object. See
    ...
    this.each(function () {
      ...
      const o = {};
      let oInit = len > 1 // optimisation for single table case
        ? _fnExtend(o, options, true)
        : options;

      /* global oInit,_that,emptyInit */
      let i = 0; let iLen; let j; let jLen; let k; let kLen;
      let sId = this.getAttribute('id');
      let bInitHandedOff = false;
      const { defaults } = DataTable;
      const $this = $(this);
      ...
      if (oInit.iDeferLoading !== null) {
        ...
        const tmp = Array.isArray(oInit.iDeferLoading);
        oSettings._iRecordsDisplay = tmp ? oInit.iDeferLoading[0] : oInit.iDeferLoading;
        oSettings._iRecordsTotal = tmp ? oInit.iDeferLoading[1] : oInit.iDeferLoading;
      }

      /* Language definitions */
      const { oLanguage } = oSettings;
      ...
      if (rowOne.length) {
        ...
        $(rowOne[0]).children('th, td').each((i, cell) => {
  ...
  let _ext; // DataTable.ext
  let _Api; // DataTable.Api
  let _api_register; // DataTable.Api.register
  let _api_registerPlural; // DataTable.Api.registerPlural
  ...
  const _re_formatted_numeric = /['\u00A0,$£€¥%\u2009\u202F\u20BD\u20a9\u20BArfkɃΞ]/gi;
  ...
  const _pluck_order = function (a, order, prop, prop2) {
    const out = [];
    let i = 0; const ien = order.length;
  ...
  /**
	 ...
	 * @ignore
	 */
  const _areAllUnique = function (src) {
    if (src.length < 2) {
      return true;
    }

    const sorted = src.slice().sort();
    let last = sorted[0];

    for (let i = 1, ien = sorted.length; i < ien; i++) {
      if (sorted[i] === last) {
        return false;
      }
  ...
  const _includes = function (search, start) {
    if (start === undefined) {
      start = 0;
    }
    ...
    set(source) {
      if ($.isPlainObject(source)) {
        ...
        var setData = function (data, val, src) {
          const a = _fnSplitObjNotation(src); let b;
          const aLast = a[a.length - 1];
          let arrayNotation; let funcNotation; let o; let innerSrc;
    ...
    get(source) {
      if ($.isPlainObject(source)) {
        ...
        var fetchData = function (data, type, src) {
          let arrayNotation; let funcNotation; let out; let innerSrc;
  ...
  function _fnLanguageCompat(lang) {
    ...
    // Default mapping
    const defaultDecimal = defaults.sDecimal;
    if (defaultDecimal) {
  ...
  function _fnCompatOpts(init) {
    ...
    if (searchCols) {
      for (let i = 0, ien = searchCols.length; i < ien; i++) {
        if (searchCols[i]) {
  ...
  /**
	 ...
	 *  @param {object} oSettings dataTables settings object
	 *  @return {int} i the number of visible columns
	 *  @memberof DataTable#oApi
	 */
  function _fnVisbleColumns(oSettings) {
    ...
    $.each(oSettings.aoColumns, (i, col) => {
  ...
  /**
	 ...
	 *  @return {array} Array of indexes with matched properties
	 *  @memberof DataTable#oApi
	 */
  ...
  function _fnColumnTypes(settings) {
    ...
    let i; let ien; let j; let jen; let k; let ken;
    let col; let cell; let detectedType; let cache;
  ...
  function _fnApplyColumnDefs(oSettings, aoColDefs, aoCols, fn) {
    let i; let iLen; let j; let jLen; let k; let kLen; let def;
    ...
    if (aoColDefs) {
      ...
      for (i = aoColDefs.length - 1; i >= 0; i--) {
        ...
        let aTargets = def.targets !== undefined
          ? def.targets
          : def.aTargets;
  ...
  /**
	 ...
	 *    if nTr is.
	 *  @return {int} >=0 if successful (index of new aoData entry), -1 if failed
	 *  @memberof DataTable#oApi
	 */
  ...
  /**
	 ...
	 *  @return {array} Array of indexes for the added rows
	 *  @memberof DataTable#oApi
	 */
  ...
  /**
	 ...
	 *  @return {int} index if the node is found, null if not
	 *  @memberof DataTable#oApi
	 */
  ...
  /**
	 ...
	 *  @return {int} index if the node is found, -1 if not
	 *  @memberof DataTable#oApi
	 */
  ...
  function _fnGetCellData(settings, rowIdx, colIdx, type) {
    ...
    let cellData = col.fnGetData(rowData, type, {
      settings,
      row: rowIdx,
      col: colIdx,
    });
  ...
  function _fnSplitObjNotation(str) {
    return $.map(str.match(/(\\.|[^\.])+/g) || [''], (s) => s.replace(/\\\./g, '.'));
  }
  ...
  /**
	 ...
	 *  @param {object} oSettings dataTables settings object
	 *  @return array {array} aData Master data array
	 *  @memberof DataTable#oApi
	 */
  ...
  /**
	 ...
	 * @return {object} Object with two parameters: `data` the data read, in
  ...
  function _fnGetRowElements(settings, row, colIdx, d) {
    ...
    let name; let col; let o; let i = 0; let contents;
    const columns = settings.aoColumns;
    const objectRead = settings._rowReadObject;
    ...
    if (td) {
      ...
      for (let j = 0, jen = tds.length; j < jen; j++) {
        cellProcess(tds[j]);
      }
    }
  ...
  function _fnCreateTr(oSettings, iRow, nTrIn, anTds) {
    ...
    const cells = [];
    let nTr; let nTd; let oCol;
    let i; let iLen; let create;
  ...
  function _fnBuildHead(oSettings) {
    let i; let ien; let cell; let row; let column;
  ...
  function _fnDrawHead(oSettings, aoSource, bIncludeHidden) {
    let i; let iLen; let j; let jLen; let k; let kLen; let n; let nLocalTr;
    const aoLocal = [];
    const aApplied = [];
    const iColumns = oSettings.aoColumns.length;
    let iRowspan; let iColspan;
  ...
  function _fnReDraw(settings, holdPosition) {
    ...
    const sort = features.bSort;
    const filter = features.bFilter;
  ...
  function _fnAddOptionsHtml(oSettings) {
    ...
    let featureNode; let cOption; let nNewNode; let cNext; let sAttr; let j;
    for (let i = 0; i < aDom.length; i++) {
      ...
      // @todo Move options into their own plugins?
        ...
        for (let k = 0, kLen = aoFeatures.length; k < kLen; k++) {
  ...
  function _fnDetectHeader(aLayout, nThead) {
    ...
    let nTr; let nCell;
    let i; let k; let l; let iLen; let jLen; let iColShifted; let iColumn; let iColspan; let iRowspan;
    let bUnique;
  ...
  /**
	 ...
	 *  @return array {node} aReturn list of unique th's
	 *  @memberof DataTable#oApi
	 */
  ...
  function _fnBuildAjax(oSettings, data, fn) {
    ...
    if ($.isPlainObject(ajax) && ajax.data) {
      ...
      const newData = typeof ajaxData === 'function'
    ...
    if (oSettings.fnServerData) {
      ...
      oSettings.fnServerData.call(
        ...
        $.map(data, (val, key) => // Need to convert back to 1.9 trad format
          ({ name: key, value: val })),
        callback,
        oSettings,
      );
  ...
  /**
	 ...
	 *  @param {object} settings dataTables settings object
	 *  @return {boolean} Block the table drawing or not
	 *  @memberof DataTable#oApi
	 */
  ...
  /**
	 ...
	 * option is set.
	 *  @param {object} oSettings dataTables settings object
	 *  @return {bool} block the table drawing or not
	 *  @memberof DataTable#oApi
	 */
  function _fnAjaxParameters(settings) {
    ...
    const preSearch = settings.oPreviousSearch;
    const preColSearch = settings.aoPreSearchCols;
    let i; const data = []; let dataProp; let column; let columnSearch;
    const sort = _fnSortFlatten(settings);
    const displayStart = settings._iDisplayStart;
    const displayLength = features.bPaginate !== false
  ...
  function _fnAjaxUpdateDraw(settings, json) {
    ...
    let data = _fnAjaxDataSrc(settings, json);
  ...
  /**
	 ...
	 *  @return {node} Filter control element
	 *  @param {object} oSettings dataTables settings object
	 *  @memberof DataTable#oApi
	 */
  function _fnFeatureHtmlFilter(settings) {
    ...
    $(settings.nTable).on('search.dt.DT', (ev, s) => {
      if (settings === s) {
  ...
  function _fnFilterComplete(oSettings, oInput, iForce) {
    ...
    if (_fnDataSource(oSettings) != 'ssp') {
      ...
      for (let i = 0; i < aoPrevSearch.length; i++) {
        _fnFilterColumn(
          oSettings,
  ...
  function _fnFilterCustom(settings) {
    ...
    for (let i = 0, ien = filters.length; i < ien; i++) {
      const rows = [];
  ...
  function _fnFilter(settings, input, force, regex, smart, caseInsensitive) {
    ...
    let display; let invalidated; let i;
    const filtered = [];
  ...
  /**
	 ...
	 *  @param {bool} bCaseInsensitive Do case insensitive matching or not
	 *  @return {RegExp} constructed object
	 *  @memberof DataTable#oApi
	 */
  function _fnFilterCreateSearch(search, regex, smart, caseInsensitive) {
    ...
    if (smart) {
      /* For smart filtering we want to allow the search to work regardless of
			 ...
			 */
      const a = $.map(search.match(/"[^"]+"|[^ ]+/g) || [''], (word) => {
        if (word.charAt(0) === '"') {
          const m = word.match(/^"(.*)"$/);
          word = m ? m[1] : word;
        }
  ...
  function _fnFilterData(settings) {
    const columns = settings.aoColumns;
    let column;
    let i; let j; let ien; let jen; let filterData; let cellData; let row;
  ...
  /**
	 ...
	 *  @param {object} oSettings dataTables settings object
	 *  @return {node} Information element
	 *  @memberof DataTable#oApi
	 */
  ...
  function _fnUpdateInfo(settings) {
    ...
    const end = settings.fnDisplayEnd();
    const max = settings.fnRecordsTotal();
  ...
  function _fnInfoMacros(settings, str) {
    ...
    const vis = settings.fnRecordsDisplay();
    const all = len === -1;
  ...
  function _fnInitialise(settings) {
    let i; let iLen; const iAjaxStart = settings.iInitDisplayStart;
    const columns = settings.aoColumns; let column;
    const features = settings.oFeatures;
  ...
  /**
	 ...
	 *  @param {object} settings dataTables settings object
	 *  @return {node} Display length feature node
	 *  @memberof DataTable#oApi
	 */
  function _fnFeatureHtmlLength(settings) {
    ...
    const d2 = Array.isArray(menu[0]);
    const lengths = d2 ? menu[0] : menu;
    const language = d2 ? menu[1] : menu;
    ...
    for (let i = 0, ien = lengths.length; i < ien; i++) {
    ...
    $(settings.nTable).on('length.dt.DT', (e, s, len) => {
      if (settings === s) {
        $('select', div).val(len);
      }
    });
  ...
  /**
	 ...
	 *  @param {object} oSettings dataTables settings object
	 *  @return {node} Pagination feature node
	 *  @memberof DataTable#oApi
	 */
  function _fnFeatureHtmlPaginate(settings) {
    const
      type = settings.sPaginationType;
    const plugin = DataTable.ext.pager[type];
    const modern = typeof plugin === 'function';
    const redraw = function (settings) {
      _fnDraw(settings);
    };
    const node = $('<div/>').addClass(settings.oClasses.sPaging + type)[0];
    const features = settings.aanFeatures;
      ...
      settings.aoDrawCallback.push({
        fn(settings) {
          if (modern) {
            ...
            const buttons = plugin(page, pages);
            let i; let ien;
  ...
  /**
	 ...
	 *  @return {bool} true page has changed, false - no change
	 *  @memberof DataTable#oApi
	 */
  function _fnPageChange(settings, action, redraw) {
    let
      start = settings._iDisplayStart;
    const len = settings._iDisplayLength;
    const records = settings.fnRecordsDisplay();
  ...
  /**
	 ...
	 *  @param {object} settings dataTables settings object
	 *  @return {node} Processing element
	 *  @memberof DataTable#oApi
	 */
  ...
  /**
	 ...
	 *  @param {object} settings dataTables settings object
	 *  @return {node} Node to add to the DOM
	 *  @memberof DataTable#oApi
	 */
  ...
  function _fnScrollDraw(settings) {
    ...
    const scrollX = scroll.sX;
    const scrollXInner = scroll.sXInner;
    const scrollY = scroll.sY;
    const barWidth = scroll.iBarWidth;
    const divHeader = $(settings.nScrollHead);
    const divHeaderStyle = divHeader[0].style;
    const divHeaderInner = divHeader.children('div');
    const divHeaderInnerStyle = divHeaderInner[0].style;
    const divHeaderTable = divHeaderInner.children('table');
    const divBodyEl = settings.nScrollBody;
    const divBody = $(divBodyEl);
    const divBodyStyle = divBodyEl.style;
    const divFooter = $(settings.nScrollFoot);
    const divFooterInner = divFooter.children('div');
    const divFooterTable = divFooterInner.children('table');
    const header = $(settings.nTHead);
    const table = $(settings.nTable);
    const tableEl = table[0];
    const tableStyle = tableEl.style;
    const footer = settings.nTFoot ? $(settings.nTFoot) : null;
    const browser = settings.oBrowser;
    const ie67 = browser.bScrollOversize;
    const dtHeaderCells = _pluck(settings.aoColumns, 'nTh');
    let headerTrgEls; let footerTrgEls;
    let headerSrcEls; let footerSrcEls;
    let headerCopy; let footerCopy;
    const headerWidths = []; const footerWidths = [];
    const headerContent = []; const footerContent = [];
    let idx; let correction; let sanityWidth;
    const zeroOut = function (nSizer) {
      const { style } = nSizer;
      style.paddingTop = '0';
  ...
  function _fnApplyToChildren(fn, an1, an2) {
    let index = 0; let i = 0; const iLen = an1.length;
    let nNode1; let nNode2;
  ...
  function _fnCalculateColumnWidths(oSettings) {
    ...
    const columns = oSettings.aoColumns;
    const scroll = oSettings.oScroll;
    const scrollY = scroll.sY;
    const scrollX = scroll.sX;
    const scrollXInner = scroll.sXInner;
    const columnCount = columns.length;
    const visibleColumns = _fnGetColumns(oSettings, 'bVisible');
    let headerCells = $('th', oSettings.nTHead);
    let tableWidthAttr = table.getAttribute('width'); // from DOM element
    const tableContainer = table.parentNode;
    let userInputs = false;
    let i; let column; let columnIdx; let width; let outerWidth;
    const browser = oSettings.oBrowser;
    const ie67 = browser.bScrollOversize;
  ...
  /**
	 ...
	 *  @param {object} settings dataTables settings object
	 *  @param {int} colIdx column of interest
	 *  @return {string} max string length for each column
	 *  @memberof DataTable#oApi
	 */
  function _fnGetMaxLenString(settings, colIdx) {
    let s; let max = -1; let maxIdx = -1;

    for (let i = 0, ien = settings.aoData.length; i < ien; i++) {
  ...
  /**
	 ...
	 *  @return {string} value with css unit
	 *  @memberof DataTable#oApi
	 */
  ...
  function _fnSortFlatten(settings) {
    let
      i; let iLen; let k; let kLen;
    const aSort = [];
    const aiOrig = [];
    const { aoColumns } = settings;
    let aDataSort; let iCol; let sType; let srcCol;
    const fixed = settings.aaSortingFixed;
    const fixedObj = $.isPlainObject(fixed);
    const nestedSort = [];
  ...
  function _fnSort(oSettings) {
    let
      i; let ien; let iLen; let j; let jLen; let k; let kLen;
    let sDataType; let nTh;
    const aiOrig = [];
    const oExtSort = DataTable.ext.type.order;
    const { aoData } = oSettings;
    const { aoColumns } = oSettings;
    let aDataSort; let data; let iCol; let sType; let oSort;
    let formatters = 0;
    let sortCol;
    const displayMaster = oSettings.aiDisplayMaster;
    let aSort;
    ...
    if (_fnDataSource(oSettings) != 'ssp' && aSort.length !== 0) {
      ...
      if (formatters === aSort.length) {
        ...
        displayMaster.sort((a, b) => {
          let
            x; let y; let k; let test; let sort;
          const len = aSort.length;
          const dataA = aoData[a]._aSortData;
          const dataB = aoData[b]._aSortData;
        ...
        displayMaster.sort((a, b) => {
          let
            x; let y; let k; let l; let test; let sort; let fn;
          const len = aSort.length;
          const dataA = aoData[a]._aSortData;
          const dataB = aoData[b]._aSortData;
  ...
  function _fnSortAria(settings) {
    ...
    const { oAria } = settings.oLanguage;
    ...
    for (let i = 0, iLen = columns.length; i < iLen; i++) {
      const col = columns[i];
      const { asSorting } = col;
      const sTitle = col.ariaTitle || col.sTitle.replace(/<.*?>/g, '');
      const th = col.nTh;
  ...
  function _fnSortListener(settings, colIdx, append, callback) {
    const col = settings.aoColumns[colIdx];
    let sorting = settings.aaSorting;
    const { asSorting } = col;
    let nextSortIdx;
  ...
  function _fnSaveState(settings) {
    ...
    const state = {
      ...
      columns: $.map(settings.aoColumns, (col, i) => ({
  ...
  function _fnLoadState(settings, oInit, callback) {
    ...
    const loaded = function (state) {
      _fnImplementState(settings, state, callback);
    };

    const state = settings.fnStateLoadCallback.call(settings.oInstance, settings, loaded);
  ...
  function _fnImplementState(settings, s, callback) {
    ...
    const api = settings._bInitComplete ? new DataTable.Api(settings) : null;
    ...
    const abStateLoad = _fnCallbackFire(settings, 'aoStateLoadParams', 'stateLoadParams', [settings, s]);
  ...
  /**
	 ...
	 *  @return {object} Settings object - or null if not found
	 *  @memberof DataTable#oApi
	 */
  function _fnSettingsFromNode(table) {
    const { settings } = DataTable;
  ...
  /**
	 ...
	 *      references
	 *  @return {object} out Reference, just for convenience - out === the return.
	 *  @memberof DataTable#oApi
  ...
  const _toSettings = function (mixed) {
    let idx; let jq;
    const { settings } = DataTable;
    const tables = $.map(settings, (el, i) => el.nTable);
  ...
  _Api = function (context, data) {
    ...
    if (Array.isArray(context)) {
      for (let i = 0, ien = context.length; i < ien; i++) {
  ...
  $.extend(_Api.prototype, {
    ...
    indexOf: __arrayProto.indexOf || function (obj, start) {
      for (let i = (start || 0), ien = this.length; i < ien; i++) {
        if (this[i] === obj) {
          return i;
        }
      }
    ...
    iterator(flatten, type, fn, alwaysNew) {
      const
        a = []; let ret;
      let i; let ien; let j; let jen;
      const { context } = this;
      let rows; let items; let item;
      const { selector } = this;
  ...
  _Api.extend = function (scope, obj, ext) {
    ...
    let
      i; let ien;
    let struct;
    const methodScoping = function (scope, fn, struc) {
      return function () {
  ...
  _Api.register = _api_register = function (name, val) {
    if (Array.isArray(name)) {
      for (let j = 0, jen = name.length; j < jen; j++) {
    ...
    let
      i; let ien;
    const heir = name.split('.');
    let struct = __apiStruct;
    let key; let method;

    const find = function (src, name) {
      for (let i = 0, ien = src.length; i < ien; i++) {
        if (src[i].name === name) {
          return src[i];
        }
      }
  ...
  var __table_selector = function (selector, a) {
    if (Array.isArray(selector)) {
      return $.map(selector, (item) => __table_selector(item, a));
    }
    ...
    const nodes = $.map(a, (el, i) => el.nTable);
  ...
  _api_registerPlural('tables().nodes()', 'table().node()', function () {
    return this.iterator('table', (ctx) => ctx.nTable, 1);
  });

  _api_registerPlural('tables().body()', 'table().body()', function () {
    return this.iterator('table', (ctx) => ctx.nTBody, 1);
  });

  _api_registerPlural('tables().header()', 'table().header()', function () {
    return this.iterator('table', (ctx) => ctx.nTHead, 1);
  });

  _api_registerPlural('tables().footer()', 'table().footer()', function () {
    return this.iterator('table', (ctx) => ctx.nTFoot, 1);
  });

  _api_registerPlural('tables().containers()', 'table().container()', function () {
    return this.iterator('table', (ctx) => ctx.nTableWrapper, 1);
  });
  ...
  _api_register('page.info()', function (action) {
    ...
    const len = settings.oFeatures.bPaginate ? settings._iDisplayLength : -1;
    const visRecords = settings.fnRecordsDisplay();
    const all = len === -1;
  ...
  const _selector_run = function (type, selector, selectFn, settings, opts) {
    let
      out = []; let res;
    let a; let i; let ien; let j; let jen;
  ...
  const _selector_first = function (inst) {
    ...
    for (let i = 0, ien = inst.length; i < ien; i++) {
      if (inst[i].length > 0) {
  ...
  var _selector_row_indexes = function (settings, opts) {
    var
      i; var ien; let tmp; let a = [];
    const displayFiltered = settings.aiDisplay;
    const displayMaster = settings.aiDisplayMaster;

    const
      { search } = opts; // none, applied, removed
    const { order } = opts; // applied, current, index (original - compatibility with 1.9)
    const { page } = opts; // all, current
    ...
    if (_fnDataSource(settings) == 'ssp') {
      ...
      if (search == 'none') {
        ...
        const displayFilteredMap = {};

        for (var i = 0, ien = displayFiltered.length; i < ien; i++) {
          displayFilteredMap[displayFiltered[i]] = null;
        }

        a = $.map(displayMaster, (el) => (!displayFilteredMap.hasOwnProperty(el)
          ? el
          : null));
      }
  ...
  const __row_selector = function (settings, selector, opts) {
    ...
    const run = function (sel) {
      ...
      // Selector - node
      if (sel.nodeName) {
        const rowIdx = sel._DT_RowIndex; // Property added by DT for fast lookup
        const cellIdx = sel._DT_CellIndex;
        ...
        const host = $(sel).closest('*[data-dt-row]');
        return host.length
          ? [host.data('dt-row')]
          : [];
      }
  ...
  _api_register('rows()', function (selector, opts) {
    ...
    const inst = this.iterator('table', (settings) => __row_selector(settings, selector, opts), 1);
  ...
  _api_register('rows().nodes()', function () {
    return this.iterator('row', (settings, row) => settings.aoData[row].nTr || undefined, 1);
  });

  _api_register('rows().data()', function () {
    return this.iterator(true, 'rows', (settings, rows) => _pluck_order(settings.aoData, rows, '_aData'), 1);
  });
  ...
  _api_registerPlural('rows().indexes()', 'row().index()', function () {
    return this.iterator('row', (settings, row) => row, 1);
  });
  ...
  _api_registerPlural('rows().ids()', 'row().id()', function (hash) {
    ...
    for (let i = 0, ien = context.length; i < ien; i++) {
      for (let j = 0, jen = this[i].length; j < jen; j++) {
  ...
  _api_registerPlural('rows().remove()', 'row().remove()', function () {
    ...
    this.iterator('row', (settings, row, thatIdx) => {
      const data = settings.aoData;
      const rowData = data[row];
      let i; let ien; let j; let jen;
      let loopRow; let loopCells;
  ...
  _api_register('rows.add()', function (rows) {
    const newRows = this.iterator('table', (settings) => {
      let row; let i; let ien;
      const out = [];
  ...
  _api_register('row.add()', function (row) {
    ...
    const rows = this.iterator('table', (settings) => {
      if (row.nodeName && row.nodeName.toUpperCase() === 'TR') {
  ...
  $(document).on('plugin-init.dt', (e, context) => {
    const api = new _Api(context);
    api.on('stateSaveParams', (e, settings, data) => {
      const indexes = api.rows().iterator('row', (settings, idx) => (settings.aoData[idx]._detailsShow ? idx : undefined));

      data.childRows = api.rows(indexes).ids(true).toArray();
    });

    const loaded = api.state.loaded();
  ...
  const __details_add = function (ctx, row, data, klass) {
    ...
    var addRow = function (r, k) {
      ...
      if (Array.isArray(r) || r instanceof $) {
        for (let i = 0, ien = r.length; i < ien; i++) {
          addRow(r[i], k);
        }
        return;
      }
      ...
      if (r.nodeName && r.nodeName.toLowerCase() === 'tr') {
        ...
        const created = $('<tr><td></td></tr>').addClass(k);
        $('td', created)
          .addClass(k)
          .html(r)
  ...
  var __details_events = function (settings) {
    ...
    if (_pluck(data, '_details').length > 0) {
      ...
      api.on(colvisEvent, (e, ctx, idx, vis) => {
        ...
        // a colspan)
        let row; const visible = _fnVisbleColumns(ctx);
  ...
  const __columnData = function (settings, column, r1, r2, rows) {
    const a = [];
    for (let row = 0, ien = rows.length; row < ien; row++) {
  ...
  const __column_selector = function (settings, selector, opts) {
    const
      columns = settings.aoColumns;
    const names = _pluck(columns, 'sName');
    const nodes = _pluck(columns, 'nTh');
    ...
    const run = function (s) {
        ...
        switch (match[2]) {
          ...
          case 'visible':
            ...
            if (idx < 0) {
              ...
              const visColumns = $.map(columns, (col, i) => (col.bVisible ? i : null));
      ...
      const host = $(s).closest('*[data-dt-column]');
      return host.length
        ? [host.data('dt-column')]
        : [];
    };
  ...
  const __setColumnVis = function (settings, column, vis) {
    const
      cols = settings.aoColumns;
    const col = cols[column];
    const data = settings.aoData;
    let row; let cells; let i; let ien; let tr;
  ...
  _api_register('columns()', function (selector, opts) {
    ...
    const inst = this.iterator('table', (settings) => __column_selector(settings, selector, opts), 1);
  ...
  _api_registerPlural('columns().header()', 'column().header()', function (selector, opts) {
    return this.iterator('column', (settings, column) => settings.aoColumns[column].nTh, 1);
  });

  _api_registerPlural('columns().footer()', 'column().footer()', function (selector, opts) {
    return this.iterator('column', (settings, column) => settings.aoColumns[column].nTf, 1);
  });
  ...
  _api_registerPlural('columns().dataSrc()', 'column().dataSrc()', function () {
    return this.iterator('column', (settings, column) => settings.aoColumns[column].mData, 1);
  });

  _api_registerPlural('columns().cache()', 'column().cache()', function (type) {
    return this.iterator('column-rows', (settings, column, i, j, rows) => _pluck_order(
      settings.aoData,
      rows,
  ...
  _api_registerPlural('columns().nodes()', 'column().nodes()', function () {
    return this.iterator('column-rows', (settings, column, i, j, rows) => _pluck_order(settings.aoData, rows, 'anCells', column), 1);
  });

  _api_registerPlural('columns().visible()', 'column().visible()', function (vis, calc) {
    const that = this;
    const ret = this.iterator('column', (settings, column) => {
      if (vis === undefined) {
    ...
    if (vis !== undefined) {
      this.iterator('table', (settings) => {
        ...
        that.iterator('column', (settings, column) => {
  ...
  const __cell_selector = function (settings, selector, opts) {
    ...
    const allCells = $(_flatten([], cells));
    let row;
    const columns = settings.aoColumns.length;
    let a; let i; let ien; let j; let o; let host;
    ...
    const run = function (s) {
      ...
      const jqResult = allCells
        .filter(s)
        .map((i, el) => ({ // use a new object, in case someone changes the values
  ...
  _api_register('cells()', function (rowSelector, columnSelector, opts) {
    ...
    // Row + column selector
    const columns = this.columns(columnSelector, internalOpts);
    const rows = this.rows(rowSelector, internalOpts);
    let i; let ien; let j; let jen;

    const cellsNoOpts = this.iterator('table', (settings, idx) => {
      const a = [];
    ...
    const cells = opts && opts.selected
  ...
  _api_registerPlural('cells().nodes()', 'cell().node()', function () {
    return this.iterator('cell', (settings, row, column) => {
      const data = settings.aoData[row];
  ...
  _api_register('cells().data()', function () {
    return this.iterator('cell', (settings, row, column) => _fnGetCellData(settings, row, column), 1);
  });

  _api_registerPlural('cells().cache()', 'cell().cache()', function (type) {
    type = type === 'search' ? '_aFilterData' : '_aSortData';

    return this.iterator('cell', (settings, row, column) => settings.aoData[row][type][column], 1);
  });

  _api_registerPlural('cells().render()', 'cell().render()', function (type) {
    return this.iterator('cell', (settings, row, column) => _fnGetCellData(settings, row, column, type), 1);
  });
  ...
  /**
	 ...
	 *  @return {boolean} true if this version of DataTables is greater or equal to
  ...
  DataTable.versionCheck = DataTable.fnVersionCheck = function (version) {
    ...
    for (let i = 0, iLen = aThat.length; i < iLen; i++) {
  ...
  /**
	 ...
	 *  @return {boolean} true the table given is a DataTable, or false otherwise
	 *  @static
	 *  @dtopt API-Static
	 *
	 *  @example
  ...
  /**
	 ...
	 *  @return {array} Array of `table` nodes (not DataTable instances) which are
	 *    DataTables
	 *  @static
	 *  @dtopt API-Static
	 *
  ...
  DataTable.tables = DataTable.fnTables = function (visible) {
    ...
    const a = $.map(DataTable.settings, (o) => {
  ...
  $.each(['on', 'one', 'off'], (i, key) => {
    _api_register(`${key}()`, function (/* event, handler */) {
      ...
      args[0] = $.map(args[0].split(/\s/), (e) => (!e.match(/\.dt\b/)
        ? `${e}.dt`
        : e)).join(' ');
  ...
  _api_register('destroy()', function (remove) {
    ...
    return this.iterator('table', (settings) => {
      ...
      const rows = $.map(settings.aoData, (r) => r.nTr);
      let i; let ien;
  ...
  $.each(['column', 'row', 'cell'], (i, type) => {
    _api_register(`${type}s().every()`, function (fn) {
      const { opts } = this.selector;
      const api = this;

      return this.iterator(type, (settings, arg1, arg2, arg3, arg4) => {
        // Rows and columns:
        //  arg1 - index
        //  arg2 - table counter
        //  arg3 - loop counter
  ...
  _api_register('i18n()', function (token, def, plural) {
    const ctx = this.context[0];
    let resolved = _fnGetObjectDataFn(token)(ctx.oLanguage);
  ...
  DataTable.models.oSettings = {
    ...
    fnDisplayEnd() {
      ...
      const start = this._iDisplayStart;
      const calc = start + len;
      const records = this.aiDisplay.length;
      const features = this.oFeatures;
      const paginate = features.bPaginate;
  ...
  function _numbers(page, pages) {
    let
      numbers = [];
    const buttons = extPagination.numbers_length;
  ...
  $.extend(true, DataTable.ext.renderer, {
    pageButton: {
      _(settings, host, idx, buttons, page, pages) {
        ...
        let btnDisplay; let btnClass; let counter = 0;

        var attach = function (container, buttons) {
          let i; let ien; let node; let button; let tabIndex;
          const disabledClass = classes.sPageButtonDisabled;
  ...
  $.extend(true, DataTable.ext.renderer, {
    header: {
      _(settings, cell, column, classes) {
        // No additional mark-up required
        // Attach a sort listener to update on sort - note that using the
        // `DT` namespace will allow the event to be removed automatically
        // on destroy, while the `dt` namespaced event is the one we are
        // listening for
        $(settings.nTable).on('order.dt.DT', (e, ctx, sorting, columns) => {
      ...
      jqueryui(settings, cell, column, classes) {
        $('<div/>')
  ...
  const __htmlEscapeEntities = function (d) {
    if (Array.isArray(d)) {
      d = d.join(',');
    }
  ...
  /**
	 ...
	 */
  DataTable.render = {
    number(thousands, decimal, precision, prefix, postfix) {
      return {
        display(d) {
